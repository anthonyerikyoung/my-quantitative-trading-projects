...
def get_sp500_tickers():
    ...
    df = pd.read_html(StringIO(str(table)))[0]  # this is already a DataFrame
    tickers = list(df['Symbol'])  # access the 'Symbol' column directly
    return tickers
...
def get_history(ticker, period_start, period_end, granularity="1d", tries=0):
    ...
    try:
        ...
        if tries<5:
            return get_history(tickers, period_start, period_end, granularity, tries+1)
        return pd.DataFrame()
...
alpha1 = Alpha1(insts=tickers, dfs=tickers_dfs, start=period_start, end=period_end)
